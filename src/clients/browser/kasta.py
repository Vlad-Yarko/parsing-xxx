...
class KastaClient(ProductClientBrowser):
    ...
    async def get_products(self, product: str, **filters: Union[str, Enum]) -> str:
        ...
        await page.goto(url=URLEnum.BASE.value + URLEnum.PRODUCTS.value + f"?q={product}", timeout=60000)
    ...
    async def get_product(self, product: str) -> str:
        page, context, browser = await self.create_page()
        await page.goto(url=URLEnum.BASE.value + URLEnum.PRODUCT.value + product, timeout=80000)
        content = await page.content()
        await self.close_page(browser, context, page)
        return content
